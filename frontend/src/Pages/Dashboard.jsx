...
// ---------------------------------------------------------------------------
// Classificação clínica para badges dos KPIs (claude)
// ---------------------------------------------------------------------------
...
export default function Dashboard() {
  ...
  const fetchEvolution = async (pacienteId) => {
    console.log('fetchEvolution chamado com:', pacienteId, startDate, endDate);
  ...
  // ---------------------------------------------------------------------------
  // KPI Card com trend + badge de classificação (claude) + layout responsivo (melhorias-ux)
  // ---------------------------------------------------------------------------
  const KPICard = ({ title, value, icon: IconComponent, colorKey = 'primary', trend, badge }) => {
    const theme = useTheme();
    const colorDark = theme.palette[colorKey]?.dark || theme.palette[colorKey]?.main || theme.palette.primary.main;
  ...
  // ---------------------------------------------------------------------------
  // Gráfico de evolução do peso — SVG responsivo (melhorias-ux)
  // ---------------------------------------------------------------------------
  const ResultChartCard = () => {
    ...
    const pesoData = evoSorted.map(e => e.peso ?? 0);
    const evoLabels = evoSorted.map((e, idx) => `${idx + 1}° consulta`);
    const evoDates = evoSorted.map(e => {
    ...
    const width = 1100, height = 350, padding = 50, rightPadding = 100;
    ...
    return (
      <Card sx={{ p: { xs: 1.5, md: 2 }, boxShadow: 3, mb: 2 }}>
        ...
        <Box sx={{ width: '100%', overflow: 'hidden' }}>
          <Box sx={{ width: '100%', minHeight: { xs: 240, sm: 280 }, aspectRatio: '1100 / 350', position: 'relative', overflow: 'hidden' }}>
            <svg
              width="100%"
              height="100%"
            ...
            >
              ...
              <polyline fill="none" stroke={primary} strokeWidth="4" points={points} />
              {pesoData.map((peso, i) => (
  ...
  // ---------------------------------------------------------------------------
  // Histórico em abas — scrollable (melhorias-ux)
  // ---------------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------------
  // Calendário / filtro de período — responsivo (melhorias-ux)
  // ---------------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------------
  // Layout principal — Grid responsivo (melhorias-ux) + botão PDF (claude)
  // ---------------------------------------------------------------------------
      ...
      <Container maxWidth="xl" sx={{ px: { xs: 1, sm: 2, md: 3 } }}>
        <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', width: '100%' }}>
          <Box sx={{ mb: 2, width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            <Typography variant="h4" fontWeight="bold" sx={{ mb: 2, padding: 3, textAlign: 'center' }}>
            ...
            <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap', justifyContent: 'center', width: '100%', alignItems: 'center' }}>
              ...
              <Button
                ...
                sx={{ height: 48 }}
              >
                {pdfLoading ? 'Gerando PDF...' : 'Baixar Relatório'}
              </Button>
  ...
  return (
    <Box sx={{ display: 'flex', minHeight: '100vh', background: 'linear-gradient(135deg, #f8fff9 0%, #e8f5e9 100%)', width: '100%' }}>
      <CssBaseline />
      <Box component="main" sx={{ flexGrow: 1, width: '100%', bgcolor: 'transparent', minHeight: '100vh', p: 2 }}>
        <MainContent />
      </Box>
    </Box>
  );
